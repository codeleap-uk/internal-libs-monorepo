...
const SelectItem:React.FC<SelectRenderFNProps<any> & { iconName?:string }>  = ({styles, iconName,  onPress, label, inList}) => {
  ...
  >
    ...
    {
      !inList &&
      <Icon name={iconName as IconPlaceholder} style={styles.buttonIcon as any}/>
    }
  </Touchable>
}
...
export const CustomSelect:React.FC<CustomSelectProps<any>> = <T extends string | number = string>(selectProps:CustomSelectProps<T>) => {
  ...
  const currentOption = isValueEmpty ? { label: placeholder, value }  : { label: optionLabelMap[value], value}
  ...
  >
    {
      options.map((item) => (
        <Item
          ...
          key={item.value}
          styles={{
            itemText: getStyles('itemText', { ...optionalObject(item.value === value, variantStyles['itemText:selected'], {}) }),
            itemWrapper: getStyles('itemWrapper', { ...optionalObject(item.value === value, variantStyles['itemWrapper:selected'], {}) }),
          }}
        />),
      )
    }
