...
import { AppTheme, CommonVariantObject, DefaultVariantBuilder, DefaultVariants, DEFAULT_VARIANTS, FromVariantsBuilder, VariantProvider } from '.'
import { AnyFunction, ComponentVariants, NestedKeys, StylesOf } from '..'
import { StyleContextProps, StyleContextValue } from './types'
import { Logger } from '../tools/Logger'
export const StyleContext = createContext({} as StyleContextValue<DefaultVariants & Record<string, CommonVariantObject<any>>>)
...
>(
...
):K extends ComponentNameArg ? Record<NestedKeys<DefaultVariants[K]>, any> : Record<NestedKeys<FromVariantsBuilder<any, S>>, any> {
  ...
  const v = CV?.[name]  
  if (!v){
    throw new Error(`Could not read context stylesheets for ${name}. Ensure it's being passed to <StyleProvider /> in the variants  prop.`)
