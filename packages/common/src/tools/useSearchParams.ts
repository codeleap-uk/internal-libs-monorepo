...
export function useSearchParams<T extends Record<string, string> = Record<string, string>>(initial?: T):UseSearchParamsReturn<T> {
  ...
  useEffect(() => {
    if (window.history){
      ...
      const url = window.location.origin + window.location.pathname + `?${searchParams.current.toString()}`
