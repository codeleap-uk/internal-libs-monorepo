...
import {  QueryState, queryStatuses, RoutesOf, UseApiArgs, UseApiReturn, UseApiState } from './types'

const getQueryStatusBooleans = (setTo) => Object.fromEntries(queryStatuses.map(s => [`is${capitalize(s)}`, s === setTo])) 
...
function routesStateReducer(state:Record<string, QueryState<Error>>, action: {type: string, payload: QueryState<Error> }){
...
(args:UseApiArgs<T, R>): RETURN { 
  ...
  const [queryStates, dispatch] = useReducer(routesStateReducer, Object.keys(routes), initRouteState)
  ...
  const initialRoutePromises = Object.fromEntries(Object.keys(routes).map((name) => [name, null]))
  ...
  const routeInterfaces = Object.fromEntries( Object.keys(routes).map((name) => {
