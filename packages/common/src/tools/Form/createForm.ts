...
function buildInitialFormState<T extends Form.FieldsMap>(name:string, form: T, inside = []){
  ...
  for (const [k, value] of Object.entries(form)){
    ...
    if (type === 'composite'){
      ...
      const { props: subFieldProps, state } = buildInitialFormState(name, value.fields, fieldPathParts)
      fieldValue = state
