...
export function arePropsEqual<A, B>(previous:A, next:B, options:ArePropsEqualOptions<A & B>) {
  ...
  for (const c of check) {
    ...
    if (!propsAreEqual) {
      const logArgs:LogFunctionArgs = ['Props not equal', { item: c, nextItem, prevItem, previous, next, check }, 'arePropsEqual']
