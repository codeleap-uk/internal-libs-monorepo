...
If you've made changes to both a submodule and a package in the Internal-libs monorepo, the best option is to create a new branch with the same name in the monorepo repository. This will allow you to work on both the submodule and other things in the monorepo, simultaneously, while keeping them separated in different branches.
